...
GROUP1_EXTRA = ['V_f_pct', 'RI', 'Lf_per_Df',
                'sqrt_fc',     # √f'c — ACI/Eurocode shear form, r_tau=0.55
                'rho_x_fc',    # ρ×f'c — Zsutty interaction term, r_tau=0.66
                'RI_sqrt_fc',  # RI×√f'c — fiber-matrix bond, r_tau=0.41
                'd_neg13',     # d^{-1/3} — Bazant size effect, r_tau=0.33
                ]
GROUP2_EXTRA = ['V_f_pct', 'RI', 'Lf_per_Df',
                'fsp_MPa', 'ft_direct_MPa', 'fr_MPa',
                'fR1_MPa', 'fR2_MPa', 'fR3_MPa', 'fR4_MPa',
                'sqrt_fc',     # √f'c — ACI/Eurocode shear form, r_tau=0.55
                'rho_x_fc',    # ρ×f'c — Zsutty interaction term, r_tau=0.66
                'RI_sqrt_fc',  # RI×√f'c — fiber-matrix bond, r_tau=0.41
                'd_neg13',     # d^{-1/3} — Bazant size effect, r_tau=0.33
...
FEATURE_GROUPS = {
    ...
    "Residual_str": ['fsp_MPa', 'ft_direct_MPa', 'fr_MPa',
                     'fR1_MPa', 'fR2_MPa', 'fR3_MPa', 'fR4_MPa'],
    "Derived":      ['sqrt_fc', 'rho_x_fc', 'RI_sqrt_fc', 'd_neg13'],
...
FEATURE_LABELS = {
    ...
    'sqrt_fc':           r"$\sqrt{f'_c}$ (MPa$^{0.5}$)",
    'rho_x_fc':          r"$\rho \times f'_c$",
    'RI_sqrt_fc':        r"$RI \times \sqrt{f'_c}$",
    'd_neg13':           r"$d^{-1/3}$ (mm$^{-1/3}$)",
    # Targets
    'V_u_KN':            r"$V_u$ (kN)",
    'Vu_KN':             r"$V_u$ (kN)",
    'Vu_kN':             r"$V_u$ (kN)",
...
FS_TAGS = {
    ...
    'fR1_MPa': 'fR1', 'fR2_MPa': 'fR2', 'fR3_MPa': 'fR3', 'fR4_MPa': 'fR4',
}
...
C = {
    # Backgrounds
    "bg":        "#F4F6FA",   # main window background
    "panel":     "#FFFFFF",   # header / section panels
    "card":      "#FFFFFF",   # card backgrounds
    "border":    "#E2E8F0",   # card borders / separators
    # Accent
    "accent":    "#2563EB",   # blue — section titles, active elements
    "accent2":   "#059669",   # green — Group 2 / success highlights
    "accent_lt": "#EFF6FF",   # light blue fill for tip boxes
    # Status
    "warn":      "#D97706",   # amber — running / warnings
    "danger":    "#DC2626",   # red   — errors / stop button
    "ok":        "#16A34A",   # green — done
    "running":   "#D97706",   # amber — in-progress
    # Text
    "text":      "#0F172A",   # primary text (near-black)
    "text_dim":  "#64748B",   # secondary / dim text
    "text_inv":  "#FFFFFF",   # inverse (on dark buttons)
    # Buttons
    "btn_bg":    "#2563EB",
    "btn_fg":    "#FFFFFF",
    "btn_hover": "#1D4ED8",
    # Log panel (keep dark for readability of code output)
    "log_bg":    "#0F172A",
    "log_fg":    "#94A3B8",
    # Input fields
    "entry_bg":  "#F8FAFC",
...
def harmonize_sfrc(df):
    ...
    aliases = {
        ...
        'fR1_MPa':       ['venk_fR1_eq_Mpa','venk_fR1_eq_MPa','fR1_MPa','f_R1_MPa'],
        'fR2_MPa':       ['venk_fR2_eq_Mpa','venk_fR2_eq_MPa','fR2_MPa','f_R2_MPa'],
        'fR3_MPa':       ['venk_fR3_eq_Mpa','venk_fR3_eq_MPa','fR3_MPa','f_R3_MPa'],
        'fR4_MPa':       ['venk_fR4_eq_Mpa','venk_fR4_eq_MPa','fR4_MPa','f_R4_MPa'],
    }
    df = _rename_first_existing(df, aliases)
...
def _pred_aci544(df):
    ...
    for _,r in df.iterrows():
        ...
        # ------------------------------------------------------------
        # Priority:
        #   1) Direct f_ut_FRC column, if added to the CSV later
        #   2) MC2010/fib simplified rigid-plastic model: f_Ftu-FRC=f_R3/3
        #   3) ASTM C1609/RILEM-type approximation: f_ut-FRC=0.37*f_150^D
        #      If f150D_MPa does not exist, fR1_MPa is used only as a rough proxy.
        #   4) Flexural strength fallback: f_ut-FRC ≈ 0.75*f_r
        #   5) Fiber-factor empirical fallback for Group1
        # ------------------------------------------------------------
        fut = np.nan
        ...
        # MC2010/fib 2013 simplified rigid-plastic model from EN 14651
        elif pd.notna(r.get('fR3_MPa', np.nan)) and float(r.get('fR3_MPa')) > 0:
            fut = float(r['fR3_MPa']) / 3.0

        # ASTM C1609/C1609M or equivalent deflection-based residual strength
        elif pd.notna(r.get('f150D_MPa', np.nan)) and float(r.get('f150D_MPa')) > 0:
            fut = 0.37 * float(r['f150D_MPa'])
        elif pd.notna(r.get('f_150D_MPa', np.nan)) and float(r.get('f_150D_MPa')) > 0:
            fut = 0.37 * float(r['f_150D_MPa'])
        elif pd.notna(r.get('fD150_MPa', np.nan)) and float(r.get('fD150_MPa')) > 0:
            fut = 0.37 * float(r['fD150_MPa'])

        # Rough proxy: fR1_MPa는 EN 14651 기준 CMOD=0.5mm 잔류 휨강도로
        # ASTM C1609의 처짐 기반 f150D와 시험 기반이 달라 0.37 계수를 그대로
        # 적용할 수 없음(BUG #3 수정: 해당 분기 제거).
        # fR1이 있을 때는 fr_MPa fallback으로 내려가도록 순서를 유지.
...
def run_experiment(exp_name, cfg, df_rc, use_rc, ml_models, lit_models, log_fn,
    ...
    for target_name in ['V_u_KN','tau_u_MPa']:
        ...
        #   Group2의 잔류강도 피처(fsp, ft_direct, fr, fR1-4)에 서로 다른 median이
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr, text="RC → SFRC  Heterogeneous Transfer Learning",
                 font=FONT_TITLE, bg=C["panel"], fg=C["text"]).pack(pady=(14, 2))
        tk.Label(hdr,
                 text="Target-specific source optimization (Vu & τu)  •  SHAP analysis  •  CSV/plot export",
                 font=FONT_SMALL, bg=C["panel"], fg=C["text_dim"]).pack(pady=(0, 12))

        # ── Slim accent bar ────────────────────────────────────────
        tk.Frame(self, bg=C["accent"], height=3).pack(fill="x")

        # ── Scrollable body ────────────────────────────────────────
        body_outer = tk.Frame(self, bg=C["bg"])
        ...
        def _wheel(event):
            if IS_WIN: canvas.yview_scroll(int(-1*(event.delta/120)), "units")
        ...
        for i, (label, var, badge_attr, browse_fn, is_folder) in enumerate([
            ...
            _FlatBtn(row, text="Browse…", command=browse_fn,
                     ...
                     font=FONT_SMALL, padx=10, pady=4).pack(side="left")

            if is_folder:
                # Extra: "New folder name" inline entry
                ...
                self._folder_name_var.trace_add("write",
                    lambda *a: self._sync_out_path())

            var.trace_add("write", lambda *a: self._check_files())

        # Tip box
        tip = tk.Frame(pc, bg=C["accent_lt"],
                       highlightbackground=C["accent"], highlightthickness=1)
        tip.pack(fill="x", padx=8, pady=(4, 10))
        tk.Label(tip,
                 text="  Tip:  The output folder is created at the path shown above."
                      "  Change 'Folder name' to auto-update the path, or use Browse to pick a parent directory.",
                 font=FONT_SMALL, bg=C["accent_lt"], fg=C["accent"],
                 anchor="w", wraplength=820, justify="left").pack(fill="x", padx=6, pady=6)
        ...
        tk.Label(log_hdr, text="Execution Log", font=FONT_SMALL,
        ...
        # Log text widget (keep dark background for terminal readability)
        self._log_box = tk.scrolledtext.ScrolledText(
            rc, height=18, font=FONT_MONO,
    ...
    def _section(self, title, parent):
        f = tk.Frame(parent, bg=C["bg"]); f.pack(fill="x", padx=16, pady=(16, 4))
        ...
        _sep(f, color=C["border"]).pack(fill="x", pady=(4, 0))

    def _card(self, parent):
        f = tk.Frame(parent, bg=C["card"], padx=6, pady=6,
                     highlightbackground=C["border"], highlightthickness=1)
