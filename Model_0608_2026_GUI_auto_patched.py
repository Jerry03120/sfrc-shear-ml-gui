...
def run_experiment(exp_name, cfg, df_rc, use_rc, ml_models, lit_models, log_fn,
    ...
    lit_pred_df = get_lit_preds(df_target, use_kwak=USE_KWAK)
    artifacts = {}
...
def run_workflow(all_arts, results_root, ml_models, use_rc, log_fn):
    ...
    for exp_name, arts in all_arts.items():
        for tn in ['V_u_KN','tau_u_MPa']:
            ...
            lit_list = (GROUP1_EXISTING_LITERATURE
                        if exp_name=='Group1' else GROUP2_REFERENCE_LITERATURE)
            export_htl_vs_lit_csv(
                art['summary'], tn, [art['best_model']], lit_list,
                final_ratio, wf_dir / f'08_HTL_vs_Lit_CSV', exp_name)
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr, text="RC → SFRC  Heterogeneous Transfer Learning",
                 font=("Arial", 23, "bold"), bg=C["panel"], fg=C["text"]).pack(pady=(7, 1))
        tk.Label(hdr,
                 text="RC → SFRC Heterogeneous Transfer Learning  •  SHAP analysis  •  CSV/plot export",
                 font=("Arial", 15), bg=C["panel"], fg=C["text_dim"]).pack(pady=(0, 5))
        ...
        style.configure("TNotebook.Tab",     background=C["border"], foreground=C["text"],
                        font=("Arial", 16, "bold"), padding=[22, 7])
        ...
        def _col_block(parent, title, color, rows, col):
            ...
            tk.Label(frm, text=title, font=FG, bg=color, fg="white",
                     anchor="w", padx=9, pady=7).pack(fill="x")
            ...
            for txt, w in colspec:
                tk.Label(hdr, text=txt, width=w, font=FH, bg="#E8EEF7",
                         fg=C["text"], anchor="w", padx=4, pady=5).pack(side="left")
        ...
        cols_frame.pack(fill="x", expand=False)
        ...
        tk.Label(ig_outer,
                 ...
                 anchor="w", padx=8, pady=5).pack(fill="x", pady=(8, 0))
        ...
        pr_cv.bind("<Configure>", lambda e: pr_cv.itemconfig(pr_cw, width=e.width))
        ...
        for gname, gcol in [("Group 1", C["accent2"]), ("Group 2", "#7C3AED")]:
            tk.Radiobutton(gr_row, text=gname,
                           ...
                           font=(FA,14), bg=C["card"], fg=C["text"],
        ...
        tk.Label(mc2, text="Step 2 — Load Saved Model", font=(FA,16,"bold"),
                 bg=C["card"], fg=C["accent"], anchor="w").pack(fill="x")
        tk.Label(mc2,
        ...
        res_card = tk.Frame(pr_sf, bg=C["card"], padx=6, pady=5,
                            highlightbackground=C["border"], highlightthickness=1)
        res_card.pack(fill="x", padx=10, pady=(3, 5))
        tk.Label(res_card, text="Step 4 — Predict", font=(FA,16,"bold"),
                 bg=C["card"], fg=C["accent"], anchor="w").pack(fill="x")
        ...
        res_boxes.pack(fill="x", padx=28, pady=(0,1))
        self._pred_res_lbl = {}
        for rkey, rlbl, rcol in [
            ...
            box.pack(side="left", fill="both", expand=True, padx=12, pady=1)
            tk.Label(box, text=rlbl, font=(FA,15,"bold"),
                     bg=C["accent_lt"], fg=rcol).pack(pady=(2,0))
        ...
        self._pred_note_lbl.pack(fill="x", padx=12, pady=(0,1))
                ...
                # ════════════════════════════════════════════════════════════
        # TAB 3 — Preview  (reads Run output files)
        # ════════════════════════════════════════════════════════════
        pv_top = tk.Frame(tab_prev, bg=C["card"],
                          highlightbackground=C["border"], highlightthickness=1)
        pv_top.pack(fill="x", padx=16, pady=(12, 4))

        # ── Row 1: Group + Target ──────────────────────────────────
        row1 = tk.Frame(pv_top, bg=C["card"])
        row1.pack(fill="x", padx=12, pady=(10, 4))

        tk.Label(row1, text="Group:", font=("Arial", 17, "bold"),
                 bg=C["card"], fg=C["text"]).pack(side="left")
        self._preview_group_var = tk.StringVar(value="Group 1")
        for gname in ["Group 1", "Group 2"]:
            tk.Radiobutton(row1, text=gname, variable=self._preview_group_var,
                           value=gname, font=("Arial", 17),
                           bg=C["card"], fg=C["text"], activebackground=C["card"],
                           selectcolor=C["card"], highlightthickness=0,
                           cursor="hand2").pack(side="left", padx=(8, 0))

        tk.Label(row1, text="   Target:", font=("Arial", 17, "bold"),
                 bg=C["card"], fg=C["text"]).pack(side="left", padx=(20, 0))
        self._preview_target_var = tk.StringVar(value="V_u (kN)")
        for tname in ["V_u (kN)", "tau_u (MPa)"]:
            tk.Radiobutton(row1, text=tname, variable=self._preview_target_var,
                           value=tname, font=("Arial", 17),
                           bg=C["card"], fg=C["text"], activebackground=C["card"],
                           selectcolor=C["card"], highlightthickness=0,
                           cursor="hand2").pack(side="left", padx=(8, 0))

        # ── Row 2: Button + Status ──────────────────────────────────
        row2 = tk.Frame(pv_top, bg=C["card"])
        row2.pack(fill="x", padx=12, pady=(4, 10))

        self._preview_btn = _FlatBtn(row2,
                                     text="▶  Load Preview from Run Results",
                                     command=self._generate_preview,
                                     bg=C["accent2"], fg=C["btn_fg"],
                                     hover="#047857", font=("Arial", 17, "bold"),
                                     padx=18, pady=7)
        self._preview_btn.pack(side="left")
        self._preview_status = tk.Label(row2,
                                        text="Run analysis first, then click here.",
                                        font=("Arial", 15), bg=C["card"], fg=C["text_dim"])
        self._preview_status.pack(side="left", padx=12)

        self._preview_fig_frame = tk.Frame(tab_prev, bg=C["bg"])
        self._preview_fig_frame.pack(fill="both", expand=True, padx=16, pady=(0, 12))
        self._preview_canvas_widget = None
    # ── Helper widgets ──────────────────────────────────────────────
    ...
    def _generate_preview(self):
        ...
        self.after(0, lambda: self._preview_status.config(
            text="Loading from Run results…", fg=C["warn"]))
        ...
        def _worker():
            try:
                ...
                msg = str(_e)
                self.after(0, lambda m=msg: self._preview_status.config(
                    text=f"Error: {m}", fg=C["danger"]))
            finally:
                self.after(0, lambda: self._preview_btn.enable(True))

        _thr.Thread(target=_worker, daemon=True).start()

    def _run_preview_worker(self):
        """
        Reads files saved by run_experiment():
          pred_df  : 01_HeteroTL/{group}/03_predictions/pred_{target}.csv
          SHAP val : 01_HeteroTL/{group}/04_shap/all_data/
                     shap_values_{target}_Hetero_TL_XGB_avail090_all.csv
          SHAP X   : shap_X_{tag}.csv

        Shows (availability=0.90, model=Hetero_TL_XGB):
          Left  — SHAP beeswarm  (all_data, avail090)
          Right — Scatter: x=V_u,pred  y=V_u,test
                  points = unique samples (mean of 5-repeat predictions)
        """
        import matplotlib
        matplotlib.use("Agg")
        ...
        grp      = self._preview_group_var.get()
        tgt_str  = self._preview_target_var.get()
        target_name = "V_u_KN" if "kN" in tgt_str.lower() else "tau_u_MPa"
        grp_key  = "Group1" if grp == "Group 1" else "Group2"
        MODEL    = PROPOSED_MODEL   # 'Hetero_TL_XGB'
        RATIO    = 0.90
        rtag     = f"avail{int(RATIO*100):03d}"
        ...
        if not results_root.exists():
            raise FileNotFoundError(
                f"Results folder not found: {results_root} — Run analysis first.")

        exp_dir  = results_root / '01_HeteroTL' / grp_key
        pred_csv = exp_dir / '03_predictions' / f'pred_{target_name}.csv'
        shap_dir = exp_dir / '04_shap' / 'all_data'
        shap_tag = f'{target_name}_{MODEL}_{rtag}_all'
        shap_val_csv = shap_dir / f'shap_values_{shap_tag}.csv'
        shap_x_csv   = shap_dir / f'shap_X_{shap_tag}.csv'

        if not pred_csv.exists():
            raise FileNotFoundError(
                f"Prediction file not found: {pred_csv} — "
                f"Run analysis with {grp} selected first.")

        # ── Load pred_df ─────────────────────────────────────────────
        pred_df = pd.read_csv(pred_csv)
        sub = pred_df[
            (pred_df['model'] == MODEL) &
            (np.isclose(pred_df['availability_ratio'].astype(float), RATIO))
        ].copy()
        if len(sub) == 0:
            raise ValueError(
                f"No predictions for model={MODEL}, ratio={RATIO} in {pred_csv}")

        # ── Average 5-repeat predictions per unique sample ───────────
        # pred_df has a 'split_id' column; group by original sample index
        # We use y_true as the sample key (samples with identical y_true get averaged)
        # More robust: use index within the dataset if available
        grp_cols = ['y_true']
        agg = sub.groupby(grp_cols)['y_pred'].mean().reset_index()
        y_true_u = agg['y_true'].values
        y_pred_u = agg['y_pred'].values

        unit = target_unit(target_name)
        r2   = float(r2_score(y_true_u, y_pred_u))
        rmse = float(np.sqrt(mean_squared_error(y_true_u, y_pred_u)))
        mae  = float(mean_absolute_error(y_true_u, y_pred_u))
        ap_m, _, ap_cov = ap_stats(y_true_u, y_pred_u)

        # ── Load SHAP (beeswarm) ─────────────────────────────────────
        shap_vals, shap_X_df = None, None
        feat_names_full = None
        if shap_val_csv.exists() and shap_x_csv.exists():
            try:
                shap_df_raw = pd.read_csv(shap_val_csv)
                shap_X_df   = pd.read_csv(shap_x_csv)
                shap_vals   = shap_df_raw.values.astype(float)
                feat_names_full = list(shap_df_raw.columns)
            except Exception:
                shap_vals = None

        # ── Draw figure via shared renderer ──────────────────────────
        self._draw_preview_figure(
            y_true_u, y_pred_u, r2, rmse, mae, ap_m, ap_cov,
            unit, target_name, grp, tgt_str,
            shap_vals, shap_X_df, feat_names_full,
            FONT_ARIAL="Arial", MODEL=MODEL, RATIO=RATIO)


    def _compute_preview_worker(self):
        """
        Compute on-the-fly: RepeatedKFold 5x5, availability=0.90.
        Each sample predicted 5 times → averaged → unique points.
        Uses same seed formula as run_experiment.
        SHAP: computed on all_data with last-fold model.
        """
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as _plt
        ...
        tgt_str     = self._preview_target_var.get()
        target_name = "V_u_KN" if "kN" in tgt_str.lower() else "tau_u_MPa"
        csv_var     = self._g1_var if grp == "Group 1" else self._g2_var
        extra_cands = GROUP1_EXTRA if grp == "Group 1" else GROUP2_EXTRA
        use_rc      = self._use_rc.get()
        RATIO       = 0.90
        FONT_ARIAL  = "Arial"

        sfrc_path = csv_var.get()
        if not Path(sfrc_path).exists():
            raise FileNotFoundError(f"CSV not found: {sfrc_path}")

        df_sfrc = harmonize_sfrc(pd.read_csv(sfrc_path))
        if target_name not in df_sfrc.columns:
            raise ValueError(f"Target '{target_name}' not found.")
        df_sfrc = df_sfrc.dropna(subset=[target_name]).reset_index(drop=True)
        N = len(df_sfrc)

        # Feature spec
        seen = set(); all_tft = []
        for c in (BASE_COMMON + extra_cands):
            if c in df_sfrc.columns and c not in ["L_f","D_f"]                and c != target_name and c not in seen:
                seen.add(c); all_tft.append(c)

        common_feats = [c for c in BASE_COMMON if c in all_tft]
        common_idx   = [all_tft.index(c) for c in common_feats]

        imp_tgt = SimpleImputer(strategy="median")
        imp_tgt.fit(df_sfrc[all_tft])
        y_full  = df_sfrc[target_name].astype(float).values

        # RC source model
        feat_names_full = list(all_tft)
        src_best = None; use_htl = False
        if use_rc and Path(self._rc_var.get()).exists():
            df_rc = harmonize_rc(pd.read_csv(self._rc_var.get()))
            if target_name not in df_rc.columns: df_rc = add_tau(df_rc)
            df_rc = df_rc.dropna(subset=[target_name])
            if len(df_rc) >= 12 and len(common_feats) >= 2:
                imp_rc = SimpleImputer(strategy="median")
                imp_rc.fit(df_rc[common_feats])
                src_best = build_xgb() or ExtraTreesRegressor(
                    n_estimators=200, random_state=RANDOM_STATE, n_jobs=N_JOBS)
                src_best.fit(imp_rc.transform(df_rc[common_feats]),
                             df_rc[target_name].astype(float).values)
                feat_names_full = ["source_pred"] + all_tft
                use_htl = True

        def _make_Z(X):
            if use_htl and src_best is not None:
                sp = src_best.predict(X[:, common_idx]).reshape(-1, 1)
                return np.hstack([sp, X])
            return X

        # CV loop — mirrors run_experiment exactly
        pred_acc = [[] for _ in range(N)]
        shap_X_all_data, shap_mdl = None, None

        rkf = RepeatedKFold(n_splits=N_SPLITS, n_repeats=N_REPEATS,
                            random_state=RANDOM_STATE)
        split_id = 0
        for train_idx, test_idx in rkf.split(df_sfrc):
            split_id += 1
            train_df = df_sfrc.iloc[train_idx].reset_index(drop=True)
            n_ft = choose_ft(len(train_df), RATIO)
            rng  = np.random.RandomState(RANDOM_STATE + split_id + int(RATIO*1000))
            fi   = rng.choice(len(train_df), size=n_ft, replace=False)
            ft_df = train_df.iloc[fi].reset_index(drop=True)
            ...
            Xft   = imp_tgt.transform(ft_df[all_tft])
            Xtest = imp_tgt.transform(df_sfrc.iloc[test_idx][all_tft])
            yft   = ft_df[target_name].astype(float).values
            ...
            mdl = build_xgb()
            if mdl is None: mdl = build_tgt_et()
            try:
                mdl.fit(Zft, yft)
                yp = mdl.predict(Ztest)
                for lj, gi in enumerate(test_idx):
                    pred_acc[gi].append(float(yp[lj]))
                shap_mdl = mdl
            except Exception:
                pass

        # all_data SHAP matrix (last model)
        if shap_mdl is not None:
            Xa    = imp_tgt.transform(df_sfrc[all_tft])
            shap_X_all_data = _make_Z(Xa)

        # Average predictions
        y_true_u = []; y_pred_u = []
        for i in range(N):
            ps = [p for p in pred_acc[i] if np.isfinite(p)]
            if ps:
                y_true_u.append(float(y_full[i]))
                y_pred_u.append(float(np.mean(ps)))
        y_true_u = np.array(y_true_u)
        y_pred_u = np.array(y_pred_u)
        ...
        # SHAP on all_data
        shap_vals = None; shap_X_df = None
        if HAS_SHAP and shap_mdl is not None and shap_X_all_data is not None:
            try:
                import shap as _shap
                exp = _shap.TreeExplainer(shap_mdl)
                sv  = exp.shap_values(shap_X_all_data)
                if isinstance(sv, list): sv = sv[0]
                sv  = np.asarray(sv, float)
                if sv.ndim == 3: sv = sv[:, :, 0]
                if sv.shape == (shap_X_all_data.shape[0], len(feat_names_full)):
                    shap_vals = sv
                    shap_X_df = pd.DataFrame(shap_X_all_data, columns=feat_names_full)
            except Exception:
                pass

        # Draw figure (same style as _run_preview_worker)
        self._draw_preview_figure(
            y_true_u, y_pred_u, r2, rmse, mae, ap_m, ap_cov,
            unit, target_name, grp, tgt_str,
            shap_vals, shap_X_df, feat_names_full, FONT_ARIAL)

    def _draw_preview_figure(self, y_true_u, y_pred_u, r2, rmse, mae,
                              ap_m, ap_cov, unit, target_name, grp, tgt_str,
                              shap_vals, shap_X_df, feat_names_full, FONT_ARIAL,
                              MODEL="HTL-XGB", RATIO=0.90):
        """Shared figure renderer for both preview modes."""
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as _plt
        import matplotlib.colors as _mc
        ...
        has_shap = (shap_vals is not None and HAS_SHAP)
        n_cols   = 2 if has_shap else 1
        fig_w    = 15.0 if has_shap else 7.5
        fig, axes = _plt.subplots(1, n_cols, figsize=(fig_w, 7.0),
                                  dpi=130, facecolor="black")
        ...
        if has_shap:
            ...
            try:
                ...
                max_disp = min(SHAP_MAX_DISPLAY, shap_vals.shape[1])
                mean_abs = np.nanmean(np.abs(shap_vals), axis=0)
                order    = np.argsort(mean_abs)[::-1][:max_disp]
                sv_disp  = shap_vals[:, order]
                xv_disp  = shap_X_df.iloc[:, order].values if shap_X_df is not None else sv_disp
                fn_disp  = [feat_names_full[i] for i in order]
                _bc = ["#053061","#2166AC","#92C5DE","#F7F7F7","#F4A582","#D6604D","#67001F"]
                _bcmap = _mc2.LinearSegmentedColormap.from_list("bee", _bc, N=256)
                y_pos = np.arange(max_disp)
                for j in range(max_disp):
                    sv_col = sv_disp[:, max_disp-1-j]
                    xv_col = xv_disp[:, max_disp-1-j]
                    xn     = (xv_col - np.nanmin(xv_col)) / (np.nanmax(xv_col) - np.nanmin(xv_col) + 1e-12)
                    np.random.seed(42)
                    jitter = np.random.uniform(-0.3, 0.3, size=len(sv_col))
                    ax_bee.scatter(sv_col, y_pos[j]+jitter, c=xn, cmap=_bcmap,
                                   s=10, alpha=0.8, linewidths=0, zorder=3)
                ax_bee.axvline(0, color="white", linewidth=0.8, zorder=2)
                ax_bee.set_yticks(y_pos)
                ax_bee.set_yticklabels(fn_disp[::-1], fontsize=14,
                                       fontfamily=FONT_ARIAL, color="white")
                ax_bee.set_xlabel("SHAP value", fontsize=14,
                                  fontfamily=FONT_ARIAL, labelpad=5, color="white")
                ax_bee.set_title(
                    f"(a) SHAP Beeswarm | {grp} | {tgt_str} | avail={int(RATIO*100)}%",
                    fontsize=13, fontfamily=FONT_ARIAL, pad=6, color="white")
                ax_bee.tick_params(axis="x", labelsize=12, colors="white")
                ax_bee.tick_params(axis="y", length=0, colors="white", labelsize=14)
                for sp in ax_bee.spines.values():
                    sp.set_color("white"); sp.set_linewidth(0.6)
                ax_bee.xaxis.grid(True, color="#444", linewidth=0.4, linestyle=":", zorder=0)
                ax_bee.set_axisbelow(True)
                sm = _plt.cm.ScalarMappable(cmap=_bcmap, norm=_mc2.Normalize(0,1))
                sm.set_array([])
                cb = fig.colorbar(sm, ax=ax_bee, fraction=0.025, pad=0.01, aspect=30)
                cb.set_label("Feature value (low to high)", fontsize=12,
                             fontfamily=FONT_ARIAL, color="white")
                cb.ax.yaxis.set_tick_params(color="white", labelcolor="white", labelsize=11)
                cb.outline.set_edgecolor("white"); cb.outline.set_linewidth(0.5)
            except Exception as _be:
                ax_bee.text(0.5, 0.5, f"SHAP error: {_be}", ha="center", va="center",
                            transform=ax_bee.transAxes, fontsize=12, color="white")

        # ── Scatter ───────────────────────────────────────────────
        ax_sc = axes[-1]
        ax_sc.set_facecolor("black")
        vmin = max(0.0, min(float(np.nanmin(y_true_u)), float(np.nanmin(y_pred_u))) * 0.92)
        vmax = max(float(np.nanmax(y_true_u)), float(np.nanmax(y_pred_u))) * 1.08
        ax_sc.plot([vmin,vmax],[vmin*0.8,vmax*0.8], color="white", lw=0.9, ls=":", zorder=1)
        ax_sc.plot([vmin,vmax],[vmin*1.2,vmax*1.2], color="white", lw=0.9, ls=":", zorder=1)
        ax_sc.plot([vmin,vmax],[vmin,vmax], color="white", lw=1.1, ls="--", zorder=2)
        ax_sc.scatter(y_pred_u, y_true_u, marker="D", s=28, color="#CC0000",
                      edgecolors="none", alpha=0.88, zorder=3)
        metrics_txt = (f"Mean = {ap_m:.3f}\n"
                       f"COV = {ap_cov*100:.1f}%\n"
                       f"$R^2$ = {r2:.3f}\n"
                       f"RMSE = {rmse:.2f} {unit}\n"
                       f"MAE = {mae:.2f} {unit}")
        ax_sc.text(0.04, 0.97, metrics_txt, transform=ax_sc.transAxes,
                   fontsize=13, fontfamily=FONT_ARIAL,
                   va="top", ha="left", color="black",
                   bbox=dict(boxstyle="round,pad=0.5", facecolor="white", edgecolor="none"))
        ax_sc.set_xlim(vmin, vmax); ax_sc.set_ylim(vmin, vmax)
        _xu = r"$V_{u,pred}$" + f" ({unit})" if target_name=="V_u_KN" else r"$	au_{u,pred}$" + f" ({unit})"
        _yu = r"$V_{u,test}$" + f" ({unit})" if target_name=="V_u_KN" else r"$	au_{u,test}$" + f" ({unit})"
        ax_sc.set_xlabel(_xu, fontsize=15, fontfamily=FONT_ARIAL, labelpad=5, color="white")
        ax_sc.set_ylabel(_yu, fontsize=15, fontfamily=FONT_ARIAL, labelpad=5, color="white")
        panel = "(b)" if has_shap else "(a)"
        ax_sc.set_title(f"{panel} [{MODEL}] | {grp} | {tgt_str} | avail={int(RATIO*100)}%",
                        fontsize=13, fontfamily=FONT_ARIAL, pad=6, color="white")
        ax_sc.tick_params(axis="both", labelsize=12, colors="white",
                          direction="in", top=True, right=True, length=3.5, width=0.8)
        ...
        def _embed():
            for w in self._preview_fig_frame.winfo_children():
                w.destroy()
            from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg as _FCA
            ctk = _FCA(fig, master=self._preview_fig_frame)
            ctk.draw()
            ctk.get_tk_widget().pack(fill="both", expand=True)
            self._preview_canvas_widget = ctk
            _plt.close(fig)
        self.after(0, _embed)
    ...
    def _pred_build_fields(self):
        ...
        for idx, (key, lbl, unit, default, required) in enumerate(fields):
            ...
            cell.grid(row=idx//ncols, column=col, sticky="ew", padx=6, pady=0)
    ...
    def _pred_load_models(self):
        ...
        for name, var in [("V_u", self._pred_vu_var),
            ...
            p = var.get().strip()
            if not p:
                # Auto-discover from results folder
                results_root = Path(self._out_var.get())
                grp_key = "Group1" if self._pred_grp_var.get()=="Group 1" else "Group2"
                tgt = "V_u_KN" if name=="V_u" else "tau_u_MPa"
                candidate = (results_root/"01_HeteroTL"/grp_key/
                             "06_saved_model"/f"model_{tgt}.pkl")
                if candidate.exists():
                    p = str(candidate)
                    var.set(p)
    ...
    def _pred_run(self):
        ...
        if row.get("a_d", np.nan) < 2.0:
            messagebox.showerror("Input Error", "a/d must be 2.0 or higher. Please enter a value of 2.0 or higher.")
            return
        ...
        for bundle, rkey, tgt in [
            ...
            try:
                ...
                imp_tgt    = bundle["imp_tgt"]
                all_tft    = bundle["all_tft"]
        ...
        if "V_u_KN" in results and "tau_u_MPa" not in results:
            ...
            if b > 0 and d > 0:
                ...
                notes.append("τu = Vu×1000/(b×d)  [no τu model].")
